...
5. **Session Resumption / Compaction Recovery**: Inspect checkpoint state to re-anchor.

---

## 📁 Storage Location & File Conventions

**NEVER store checkpoint files in the repository working directory.**

Store checkpoints in external harness storage or temporary directories:

- **Harness State / Artifact Directory**: Dedicated agent state or scratch path provided by the environment
  (e.g., `<appDataDir>/brain/<conversation-id>/scratch/` or runtime state directory).
- **Temporary Directory**: System temporary directory (e.g., `/tmp/agent-checkpoints/<session-id>/`).
