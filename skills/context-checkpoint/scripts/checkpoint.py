...
SCHEMA_PATH = (
    Path(__file__).resolve().parent.parent / "schemas" / "checkpoint.schema.json"
)
...
def render_markdown(data: dict) -> str:
    ...
    scope_str = (
        "\n".join(f"- {s}" for s in scope_boundaries)
        if scope_boundaries
        else "- None specified"
    )
    acceptance_str = (
    ...
    if milestones:
        ...
        for m in milestones:
            ...
            milestone_rows.append(
                f"| {m_id} | {m_title} | {m_status} | {m_verify} | {m_time} |"
            )
        milestones_table = "\n".join(milestone_rows)
    else:
    ...
    next_actions_str = (
        "\n".join(f"  1. {a}" for a in next_actions)
        if next_actions
        else "  1. _None specified_"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Context Checkpoint CLI & State Synchronizer"
    )
    parser.add_argument(
        "--file",
        default=DEFAULT_CHECKPOINT_FILE,
        help=f"Path to JSON checkpoint file (default: {DEFAULT_CHECKPOINT_FILE})",
    )
    parser.add_argument(
        "--session-file",
        default=DEFAULT_SESSION_FILE,
        help=f"Path to Markdown session file (default: {DEFAULT_SESSION_FILE})",
    )

    subparsers = parser.add_subparsers(dest="subcommand", required=True)

    # Init
    p_init = subparsers.add_parser("init", help="Initialize a new checkpoint")
    p_init.add_argument("--session-id", help="Custom session ID")
    p_init.add_argument("--goal", help="Primary goal description")
    p_init.add_argument(
        "--scope", action="append", help="Scope boundary (can specify multiple)"
    )
    p_init.add_argument(
        "--criteria", action="append", help="Acceptance criteria (can specify multiple)"
    )
    p_init.add_argument(
    ...
    # Milestone
    p_milestone = subparsers.add_parser("milestone", help="Manage milestones")
    m_sub = p_milestone.add_subparsers(dest="milestone_action", required=True)

    p_m_add = m_sub.add_parser("add", help="Add a milestone")
    p_m_add.add_argument("--id", help="Milestone ID (e.g. M1)")
    p_m_add.add_argument("--title", required=True, help="Milestone title")
    p_m_add.add_argument(
        "--status",
        choices=["pending", "in_progress", "completed", "failed", "blocked"],
        default="pending",
        help="Initial status",
    )
    ...
    p_m_comp = m_sub.add_parser("complete", help="Complete and verify a milestone")
    p_m_comp.add_argument("id", help="Milestone ID to complete")
    p_m_comp.add_argument(
        "--verify-cmd", required=True, help="Command used to verify this milestone"
    )
    p_m_comp.set_defaults(func=cmd_milestone_complete)

    # Decision
    p_dec = subparsers.add_parser("decision", help="Record architectural decisions")
    d_sub = p_dec.add_subparsers(dest="decision_action", required=True)
    p_d_add = d_sub.add_parser("add", help="Add a decision")
    ...
    # Blocker
    p_blk = subparsers.add_parser("blocker", help="Record blockers or issues")
    b_sub = p_blk.add_subparsers(dest="blocker_action", required=True)
    p_b_add = b_sub.add_parser("add", help="Add a blocker")
    p_b_add.add_argument("--id", help="Blocker ID (e.g. B1)")
    p_b_add.add_argument("--desc", required=True, help="Blocker description")
    p_b_add.add_argument(
        "--status", choices=["active", "resolved", "investigating"], default="active"
    )
    ...
    p_ctx = subparsers.add_parser(
        "update-context", help="Update active working context"
    )
    p_ctx.add_argument("--step", help="Current step description")
    p_ctx.add_argument(
        "--file-add", action="append", help="Add open/modified file path"
    )
    p_ctx.add_argument(
        "--next-action", action="append", help="Next action item (can specify multiple)"
    )
    p_ctx.set_defaults(func=cmd_update_context)

    # Render
    p_render = subparsers.add_parser(
        "render", help="Render SESSION.md from .checkpoint.json"
    )
    p_render.set_defaults(func=cmd_render)

    # Validate
    p_val = subparsers.add_parser(
        "validate", help="Validate .checkpoint.json structure"
    )
